...
if(sizeof($users)>0)
{
	for($i=0;$i<sizeof($users);$i++)
	{
		...
		";
		$query="select * from rec_entries where idate >='".$today."' and status='1' and office='".$users[$i]["office"]."' order by office, idate desc, itime desc";
		if($result=mysql_query($query))
		{
		...
		<td valign='middle' align='center' width='59%'>Source</td>
		<td valign='middle' align='center' width='35%'>Orientation Date</td>
		</tr>
		";
		$query="select * from rec_entries where orientation >='".$today."' and status='3' and orientation_office='".$users[$i]["office"]."' order by orientation desc";
		if($result=mysql_query($query))
		{
			if(($numint_totalx=mysql_num_rows($result))>0)
			{

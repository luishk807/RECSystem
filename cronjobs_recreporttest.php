...
//$lsunday=date("Y-m-d", mktime(0, 0, 0, date("m"),date("d")-8,date("Y")));
$yesterday=date("Y-m-d", mktime(0, 0, 0, 7,9-2,2012));
$lsunday=date("Y-m-d", mktime(0, 0, 0, 7,9-8,2012));
$xdate=array("date1"=>$yesterday,"date2"=>$lsunday);
//end of test
//$today = date('Y-m-d');
$today='2012-07-09';
$ttoday = $xdate["date1"];
...
if(sizeof($listemails_a)>0)
{
	...
	if($result = mysql_query($query))
	{
		if(($numrows = mysql_num_rows($result))>0)
		{
			while($rows = mysql_fetch_array($result))
			{
				...
				$qx="select * from rec_phones where office='".$rows["id"]."' and date between '".$tday."' and '".$ttoday."' order by date desc";
				if($rx=mysql_query($qx))
				{
					if(($nx=mysql_num_rows($rx))>0)
					{
						...
						{
							$rox_date=fixdate_comps('invdate_s',$rox["date"]);
							$rox_caller=checkNA($rox["caller"]);
							$message .="<tr><td width='15%' align='center' valign='middle'>".$cx."</td><td width='17%' align='center' valign='middle'>".$rox_date."</td><td width='35%' height='20' align='center' valign='middle'>".$rox_caller."</td><td width='33%' align='center' valign='middle'>".$rox["tphone"]."</td></tr>";
							$cx++;
						}
					}
					else
					 	$message .="<tr style='background:#8a0604; color:#FFF;font-style:italic'><td colspan='4' align='center' valign='middle'>No Unique Phone Found</td></tr>";
				}
				else
					$message .="<tr style='background:#8a0604; color:#FFF;font-style:italic'><td colspan='4' align='center' valign='middle'>No Unique Phone Found</td></tr>";
				$message .="</table>";*/
	...
	if($result = mysql_query($query))
	{
		...
		$message .="<div style='font-size:18pt; text-align:center; text-decoration:underline; color:#00477f;'>**************************************************<br/>NO OFFICE FOUND IN SYSTEM<br/>**************************************************</div>";
	//$resultemail = sendEmail("luishk807@hotmail.com",$title,$message);
	//echo $message;

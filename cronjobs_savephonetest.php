...
$echeck[]=array("id"=>"8","email"=>"dmajor.5@familyenergy.onsip.com");
for($y=0;$y<sizeof($echeck);$y++)
{
	$fphone=array();
	$userx = getOnSip(trim($echeck[$y]["email"]));
	$str = getJunction("cdr","UserId=".$userx["userid"]."&StartDateTime=".$fdate."&EndDateTime=".$tdate."&Limit=1000","&OrderBy=DateTime");
	//print_r($str);
		...
		if(sizeof($uniqp)>0)
		{
			for($i=0;$i<sizeof($uniqp);$i++)
			{
				...
				if(!$found)
				{
					...
					//if($uniqp[$i]["xpfrom"] !="Restricted" && $uniqp[$i]["xpfrom"] !="Anonymous")
					if($xcheck !="Restricted" && $xcheck !="Anonymous" && $xcheck !='Anonymous'  && $xcheck !='unavailable')
					{
						$newcaller="";
						$newword="";
						...
						$fphone="";
						$fphone=fixOnSipPhone('family',$uniqp[$i]["xpfrom"]);
						//echo $uniqp[$i]["callerid"]." and ".$newword."<br/>";
						$query="insert ignore into rec_phones(office,caller,tphone,fphone,date)values('".$echeck[$y]["id"]."','".clean($newword)."','".$uniqp[$i]["xpfrom"]."','".$fphone."','".$today."')";
						echo $query."<br/>";
						//@mysql_query($query);
					}
				}
